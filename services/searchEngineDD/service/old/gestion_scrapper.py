...
async def google_search_async(lista_criterios_busqueda: list[tuple[str, str]], xpath: str, sess: ClientSession, dict_tracker) -> list:
    ...
    tasks = [consulta_pagina_web(sess, f'https://www.google.com/search?q=allintext%3A{quote(criterio_busqueda)}+site:gestion.pe&num=50&tbs=cdr:1,cd_min:1/1/2019&source=lnt', _header, keyword, dict_tracker) for keyword, criterio_busqueda in lista_criterios_busqueda]
...
def formatting_results(results: list, _xpath_base: str, _xpath_titulo: str, _xpath_enlace: str, _xpath_fecha: str, _xpath_fuente: str) -> list:
    ...
    for resultado, url, keyword in results:
        ...
        Nueva_Data = [
                {
                    'Fuente': resultado_content.xpath(_xpath_fuente)[0].xpath('string(.)') if resultado_content.xpath(_xpath_fuente) else 'Gestion.pe',
                    'Titulo': limpiar_titulo(resultado_content.xpath(_xpath_titulo)[0].xpath('string(.)')) if resultado_content.xpath(_xpath_titulo) else '',
                    'Fecha': parsear_fecha(resultado_content.xpath(_xpath_fecha)[0].xpath('string(.)') if resultado_content.xpath(_xpath_fecha) else ''),
                ...
                if (
                    ...
                    '/tag/' not in url and '/tags/' not in url
                )
            ]
...
async def scraping_gestion(sess: ClientSession, nombre_comercial: str, razon_social: str, entityIdNumber: str):
    ...
    try:
        ...
        logging.info(f'Requests status counter in "Gestion" from "{nombre_comercial}": {dict_tracker}')
        logging.info(f'Successfully finishing "Gestión" search process for entity "{nombre_comercial}"')
