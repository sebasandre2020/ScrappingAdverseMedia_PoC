...
async def google_search_async(lista_criterios_busqueda: list[tuple[str, str]], xpath: str, sess: ClientSession, dict_tracker) -> list:
    ...
    tasks = [consulta_pagina_web(sess, f'https://www.google.com/search?q=allintext%3A{quote(criterio_busqueda)}+site:https://larepublica.pe/&num=50&tbs=cdr:1,cd_min:1/1/2019&source=lnt', _header, keyword, dict_tracker) for keyword, criterio_busqueda in lista_criterios_busqueda]
...
async def webscraping_integridad_diario_la_republica(sess: ClientSession, nombre_comercial: str, razon_social: str, entityIdNumber: str):
    ...
    try:
        ...
        logging.info(f'Requests status counter in "La República" from "{nombre_comercial}": {dict_tracker}')
        logging.info(f'Successfully finishing "La República" search process for entity "{nombre_comercial}"')
