...
async def google_search_async(lista_criterios_busqueda: list[tuple[str, str]], xpath: str, sess: ClientSession, dict_tracker) -> list:
    ...
    tasks = [consulta_pagina_web(sess, f'https://www.google.com/search?q=allintext:{quote(criterio_busqueda)}+site:convoca.pe&num=200&tbs=cd_min:01/01/2019', _header, keyword, dict_tracker) for keyword, criterio_busqueda in lista_criterios_busqueda]
...
def formatting_results(results: list, _xpath_base: str, _xpath_titulo: str, _xpath_enlace: str, _xpath_fecha: str, _xpath_fuente: str) -> list:
    ...
    for resultado, url, keyword in results:
        ...
        Nueva_Data = [
                ...
                if (
                    ...
                    'convoca-radio' not in url and
                    len(url.split('/')) != 4  # Excluir URLs con exactamente 4 segmentos
                )
            ]
    ...
    if not df.empty:
        ...
        df = df[df['Fecha'] >= pd.Timestamp('2019-01-01')]
        df['Fecha'] = df['Fecha'].dt.strftime('%Y-%m-%d')
        #df = df.drop_duplicates(subset='URL', keep='first')

        # Convert back to list of dictionaries
        data_total = df.to_dict(orient='records')

    
    return data_total

async def scraping_convoca(sess: ClientSession, nombre_comercial: str, razon_social: str, entityIdNumber: str):
    ...
    try:
        ...
        logging.info(f'Requests status counter in "Convoca" from "{nombre_comercial}": {dict_tracker}')
        logging.info(f'Successfully finishing "Convoca" search process for entity "{nombre_comercial}"')
